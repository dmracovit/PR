...
describe("Board", function () {
  ...
  describe("Advanced Concurrency Scenarios", function () {
    ...
    it("should handle waiting chain: A controls, B waits, C waits, A releases", async function () {
      ...
      assert.strictEqual(
        charlieGotCard,
        false,
        "Charlie should still be waiting"
      );
    ...
    it("should handle player making match while others wait for one of the cards", async function () {
      ...
      assert.strictEqual(
        result,
        "failed",
        "Bob should fail when card is removed"
      );
    ...
    it("should not deadlock when two players want each others cards", async function () {
      ...
      if (aliceFailed) {
        assert.match(
          (aliceResult as PromiseRejectedResult).reason.message,
          /controlled by another player|no card at/
        );
      }
      if (bobFailed) {
        assert.match(
          (bobResult as PromiseRejectedResult).reason.message,
          /controlled by another player|no card at/
        );
      }
    ...
    it("should correctly handle three players in sequence with waiting", async function () {
      ...
      assert(
        bobView.includes("my A") || charlieView.includes("my A"),
  ...
  describe("map()", function () {
    ...
    it("should handle empty spaces correctly", async function () {
      ...
      assert(
        afterMap.includes("none"),
        "Empty spaces should remain empty after map"
      );
  ...
  describe("watch()", function () {
    ...
    it("should handle multiple concurrent watchers", async function () {
      ...
      const watch1 = board.watchForChange().then(() => {
        watcher1Resolved = true;
      });
      const watch2 = board.watchForChange().then(() => {
        watcher2Resolved = true;
      });
      const watch3 = board.watchForChange().then(() => {
        watcher3Resolved = true;
      });
    ...
    it("should not notify on control changes without face changes", async function () {
      ...
      assert(
        !watchResolved,
        "Watch should not resolve on control change alone"
      );
    ...
    it("should allow look() to interleave with watch()", async function () {
      ...
      assert(
        view2.includes("my A"),
        "Player1 should see their controlled card"
      );
